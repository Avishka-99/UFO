...
func _physics_process(delta):
	...
	if Input.is_action_pressed("ui_left"):
		...
		self.global_transform.origin=self.global_transform.origin.linear_interpolate(Vector3(self.global_transform.origin.x,self.global_transform.origin.y,-3.3),delta*speed)
...
func transition_camera3D(from: Camera, to: Camera, duration: float = 1.0) -> void:
	...
	tween.interpolate_property(Camera3D, "global_transform", Camera3D.global_transform, 
		$cam.global_transform, duration, Tween.TRANS_CUBIC, Tween.EASE_IN_OUT)
